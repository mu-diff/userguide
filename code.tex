...
For example, for a Dirichlet boundary value problem, the EFIE (\ref{EFIED}), which is based on a single-layer representation, can be built by using the function \texttt{SingleLayer} for a dense matrix version or the function \texttt{SpSingleLayer} to get a sparse representation. % function call for a Krylov subspace solver.
...
All the functions computing the integral operators are located in the \code{IntOperators/} folder.
